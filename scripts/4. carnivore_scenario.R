# Runs carnivore removal scenario
...
# Remove large (>100 kg) endothermic carnivores
remove_idx <- which(mdata_spin_up$cohorts$AdultMass > 1e5 & 
                      mdata_spin_up$cohorts$FunctionalGroupIndex == 1)
...
chrt_def$PROPERTY_Maximum.mass[2] = 100000 # set max size carnivores = 100000 g (100 kg)

dir_path <- paste0(output_folder,"/carnivore_scenario")
if (!dir.exists(dir_path)) {
  dir.create(dir_path, recursive = TRUE)
}
mdata_carnivore_scenario <- madingley_run(years = n_years, 
                                          out_dir = dir_path,
...
saveRDS(mdata_carnivore_scenario, 
        paste0(output_folder, "/mdata_carnivore_scenario.rds"))
#!/usr/bin/env Rscript
# =============================================================================
# Compute diversity maps from pre–land-use spin-up (MadingleyR)
# - Reads saved run: mdata2_spinup.rds
...
# - Writes GeoTIFFs and small preview TIFFs
#
# Author: Wenhuan
# Date:   2025-08-22
# =============================================================================
...
# This is your *default* output directory that holds mdata2_spinup.rds
spinup_dir <- "/Users/wenhuan/Documents/Conferences-Workshop/Biodiversity modeling 2025/code and data/TempMadingleyOuts_default/"
stopifnot(dir.exists(spinup_dir))

# Where to put the exported rasters/plots (can be the same as spinup_dir)
out_dir <- file.path(spinup_dir, "diversity_from_spinup")
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

# Path to the saved spin-up result
spinup_rds <- file.path(spinup_dir, "mdata2.rds")
stopifnot(file.exists(spinup_rds))

# --------------------------- 1) Load and basic checks -------------------------

mres <- readRDS(spinup_rds)
...
# You can keep or remove size-binning; it's not required for the three outputs.
...
## 3B) Biomass-weighted Shannon (abundance * individual mass)
...
# Some builds index from 0; use relative indexing to be safe
...
# Template at the model window and resolution
r_template <- rast(ext = ext(xmin, xmax, ymin, ymax),
                   resolution = gs, crs = "EPSG:4326")

# Helper to rasterize a single column from div_df
...
# Write GeoTIFFs
writeRaster(r_abund, filename = file.path(out_dir, "shannon_abundance.tif"),
            overwrite = TRUE)
writeRaster(r_biom,  filename = file.path(out_dir, "shannon_biomass.tif"),
            overwrite = TRUE)
writeRaster(r_meta,  filename = file.path(out_dir, "shannon_metabolism.tif"),
            overwrite = TRUE)

# ---------------------------- 6) Quick preview TIFFs --------------------------

tiff(file.path(out_dir, "shannon_abundance_preview.tiff"),
     width = 4, height = 4, units = "in", res = 300)
plot(r_abund, main = "Shannon (Abundance-weighted)")
dev.off()

tiff(file.path(out_dir, "shannon_biomass_preview.tiff"),
     width = 4, height = 4, units = "in", res = 300)
plot(r_biom, main = "Shannon (Biomass-weighted)")
dev.off()

tiff(file.path(out_dir, "shannon_metabolism_preview.tiff"),
     width = 4, height = 4, units = "in", res = 300)
plot(r_meta, main = "Shannon (By FunctionalGroupIndex)")
dev.off()

cat("✅ Wrote GeoTIFFs and preview TIFFs to:\n", out_dir, "\n")
# =============================================================================

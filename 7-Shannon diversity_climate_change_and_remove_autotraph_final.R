#!/usr/bin/env Rscript
# =============================================================================
# Diversity maps from final scenario (climate change + land-use)
# - Reads: mdata_final.rds
# - Outputs:
#     1) Shannon (Abundance-weighted)
#     2) Shannon (Biomass-weighted)
#     3) Shannon (By FunctionalGroupIndex, i.e., "metabolism")
# - Writes GeoTIFFs + quick preview TIFFs
#
# Author: Wenhuan
# Date:   2025-08-22
# =============================================================================
...
# Folder containing your final result mdata_final.rds
...
input_rds <- file.path(base_dir, "mdata_final.rds")
stopifnot(file.exists(input_rds))

out_dir <- file.path(base_dir, "diversity_from_final")
...
# Not required for outputs, but keeps FG x size strata consistent with earlier scripts.
...
# Robust to 0- or 1-based indexing
...
writeRaster(r_abund, file.path(out_dir, "shannon_abundance_final.tif"),       overwrite = TRUE)
writeRaster(r_biom,  file.path(out_dir, "shannon_biomass_final.tif"),         overwrite = TRUE)
writeRaster(r_meta,  file.path(out_dir, "shannon_metabolism_final.tif"),      overwrite = TRUE)

# ---------------------------- 6) Quick previews -------------------------------

tiff(file.path(out_dir, "shannon_abundance_final_preview.tiff"),
     width = 4, height = 4, units = "in", res = 300)
plot(r_abund, main = "Shannon (Abundance-weighted) — Final")
dev.off()

tiff(file.path(out_dir, "shannon_biomass_final_preview.tiff"),
     width = 4, height = 4, units = "in", res = 300)
plot(r_biom, main = "Shannon (Biomass-weighted) — Final")
dev.off()

tiff(file.path(out_dir, "shannon_metabolism_final_preview.tiff"),
     width = 4, height = 4, units = "in", res = 300)
plot(r_meta, main = "Shannon (By FunctionalGroupIndex) — Final")
dev.off()

cat("✅ Final-scenario diversity rasters written to:\n", out_dir, "\n")
# =============================================================================
